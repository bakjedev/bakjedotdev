...
public class Peek extends Mod {
    public NumberSetting mapSize = new NumberSetting("MapSize", 0.25, 1.5, 0.85, 0.01);

    public ModeSetting info = new ModeSetting("Info","All", "All", "Name", "None");
    ...
    public List<TooltipComponent> drawShulkerToolTip(MatrixStack matrices, Slot slot, int mouseX, int mouseY) {
        ...
        List<ItemStack> items = ItemContentUtil.getItemsInContainer(slot.getStack());

        if (items.stream().allMatch(ItemStack::isEmpty)) {
            return null;
        }

        int mode = this.info.getIndex();
    ...
    public void drawBookToolTip(MatrixStack matrices, Slot slot, int mouseX, int mouseY) {
        ...
        if (pages == null) {
            pages = ItemContentUtil.getTextInBook(slot.getStack());
        }
    ...
    public void drawMapToolTip(MatrixStack matrices, Slot slot, int mouseX, int mouseY) {
        ...
        float scale = this.mapSize.getValueFloat() / 1.25f;
